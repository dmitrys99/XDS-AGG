...
TYPE
  pixel_format_bgr24_ptr* = POINTER TO pixel_format_bgr24;
  pixel_format_bgr24* = RECORD(AggPixfmt24.pixel_format24)
  END;

PROCEDURE (pf: pixel_format_bgr24_ptr) ConstructI*();
BEGIN
  pf.ConstructI^();
...
PROCEDURE (pf: pixel_format_bgr24_ptr) blend_pix_bgr24(x, y: bas.int32; cr, cg, cb, alpha: bas.int8u);
VAR
  r,g,b: bas.int32;
...
BEGIN
  ...
  pf.setvalue(x, y, pf.byte_order.R, bas.agg_getbyte(r + ASH((cri - r) * a, -col.base_shift)));
  pf.setvalue(x, y, pf.byte_order.G, bas.agg_getbyte(g + ASH((cgi - g) * a, -col.base_shift)));
  pf.setvalue(x, y, pf.byte_order.B, bas.agg_getbyte(b + ASH((cbi - b) * a, -col.base_shift)));
END blend_pix_bgr24;

(* copy_or_blend_pix_bgr *)

PROCEDURE (pf: pixel_format_bgr24_ptr) copy_or_blend_pix_bgr(x, y: bas.int32; c: col.aggclr; cover: bas.int8u);
VAR
  alpha: bas.int8u;
BEGIN
  IF c.a # 0 THEN
...
PROCEDURE (pf: pixel_format_bgr24_ptr) copy_or_blend_pix_bgr1(x, y: bas.int32; c: col.aggclr);
BEGIN
  IF c.a # 0 THEN
...
PROCEDURE (pf: pixel_format_bgr24_ptr) blend_pixel*(x, y: bas.int32; c: col.aggclr; cover: bas.int8u);
BEGIN
...
PROCEDURE (pf: pixel_format_bgr24_ptr) blend_hline*(x, y, len: bas.int32; c: col.aggclr; cover: bas.int8u);
VAR
  alpha: bas.int8u;
  ax: bas.int32;
BEGIN
...
PROCEDURE (pf: pixel_format_bgr24_ptr) blend_vline*(x, y, len: bas.int32; c: col.aggclr; cover: bas.int8u);
VAR
  alpha: bas.int8u;
  ay: bas.int32;
BEGIN
...
(* «десь необходимо сделать важное замечание.
    ...
    *)
PROCEDURE (pf: pixel_format_bgr24_ptr) blend_solid_hspan*(x, y, len: bas.int32; c: col.aggclr; VAR covers: col.covers_array_ptr; cov_offset: bas.int32);
VAR
  alpha: bas.int8u;
  i,ax, alen: bas.int32;
BEGIN
...
PROCEDURE (pf: pixel_format_bgr24_ptr) blend_solid_vspan*(x, y, len: bas.int32; c: col.aggclr;
...
PROCEDURE (pf: pixel_format_bgr24_ptr) blend_color_hspan*(x, y, len: bas.int32;
...
PROCEDURE (pf: pixel_format_bgr24_ptr) blend_color_vspan*(x, y, len: bas.int32;
  VAR colors: col.colors_array_ptr; col_offset: bas.int32;
  VAR covers: col.covers_array_ptr; cov_offset: bas.int32;
       cover: bas.int8u);
VAR
...
PROCEDURE (pf: pixel_format_bgr24_ptr) copy_from*(from: rb.rendering_buffer_ptr; xdst, ydst, xsrc, ysrc, len: bas.int32);
BEGIN
  bas.agg_move(
...
PROCEDURE (pf: pixel_format_bgr24_ptr) blend_from*(from: AggPixfmtBase.pixel_formats_ptr; xdst, ydst, xsrc, ysrc, len: bas.int32; cover: bas.int8u);
VAR
  color: col.aggclr;
  alpha: bas.int8u;
