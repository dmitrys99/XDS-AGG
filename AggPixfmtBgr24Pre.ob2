...
TYPE
  pixel_format_bgr24_pre_ptr* = POINTER TO pixel_format_bgr24_pre;
  pixel_format_bgr24_pre* = RECORD(AggPixfmt24.pixel_format24)
  END;

PROCEDURE (pf: pixel_format_bgr24_pre_ptr) ConstructI*();
BEGIN
  pf.ConstructI^();
  pf.byte_order := AggPixfmtBase.bgr_order;
END ConstructI;

(* blend_pix_bgr *)

PROCEDURE (pf: pixel_format_bgr24_pre_ptr) blend_pix_bgr24_pre(x, y: bas.int32; cr, cg, cb, alpha, cover: bas.int8u);
VAR
  r,g,b: bas.int32;
BEGIN
...
END blend_pix_bgr24_pre;

PROCEDURE (pf: pixel_format_bgr24_pre_ptr) blend_pix_bgr24_pre1(x, y: bas.int32; cr, cg, cb, alpha: bas.int8u);
VAR
  r,g,b: bas.int32;
...
END blend_pix_bgr24_pre1;

(* copy_or_blend_pix_bgr *)

PROCEDURE (pf: pixel_format_bgr24_pre_ptr) copy_or_blend_pix_bgr_pre(x, y: bas.int32; c: col.aggclr; cover: bas.int8u);
VAR
  alpha: bas.int8u;
BEGIN
  IF c.a # 0 THEN
    ...
    ELSE
      pf.blend_pix_bgr24_pre1(x, y, c.r, c.g, c.b, alpha);
    END;
  END;
END copy_or_blend_pix_bgr_pre;

PROCEDURE (pf: pixel_format_bgr24_pre_ptr) copy_or_blend_pix_bgr1(x, y: bas.int32; c: col.aggclr);
BEGIN
  IF c.a # 0 THEN
    IF c.a = col.base_mask THEN
      ...
      pf.setvalue(x, y, pf.byte_order.B, c.b);
    ELSE
      pf.blend_pix_bgr24_pre1(x, y, c.r, c.g, c.b, c.a);
    END;
  END;
END copy_or_blend_pix_bgr1;


(* blend_pixel *)

PROCEDURE (pf: pixel_format_bgr24_pre_ptr) blend_pixel*(x, y: bas.int32; c: col.aggclr; cover: bas.int8u);
BEGIN
  pf.copy_or_blend_pix_bgr_pre(x, y, c, cover);
END blend_pixel;

(* blend_hline *)

PROCEDURE (pf: pixel_format_bgr24_pre_ptr) blend_hline*(x, y, len: bas.int32; c: col.aggclr; cover: bas.int8u);
VAR
  alpha: bas.int8u;
  ax: bas.int32;
BEGIN
  ...
  IF c.a # 0 THEN
    ...
    ELSE
      REPEAT
        pf.blend_pix_bgr24_pre1(ax, y, c.r, c.g, c.b, alpha);
        INC(ax);
        DEC(len);
      UNTIL len = 0;
    END;
...
PROCEDURE (pf: pixel_format_bgr24_pre_ptr) blend_vline*(x, y, len: bas.int32; c: col.aggclr; cover: bas.int8u);
VAR
  alpha: bas.int8u;
  ay: bas.int32;
BEGIN
  ...
  IF c.a # 0 THEN
    ...
    ELSE
      REPEAT
        pf.blend_pix_bgr24_pre1(x, ay, c.r, c.g, c.b, alpha);
        INC(ay);
        DEC(len);
      UNTIL len = 0;
    END;
...
(* Здесь необходимо сделать важное замечание.
    ...
    *)
PROCEDURE (pf: pixel_format_bgr24_pre_ptr) blend_solid_hspan*(x, y, len: bas.int32; c: col.aggclr; VAR covers: col.covers_array_ptr; cov_offset: bas.int32);
VAR
  alpha: bas.int8u;
  i,ax, alen: bas.int32;
BEGIN
  ...
  IF c.a # 0 THEN
    ...
    REPEAT
      ...
      IF alpha = col.base_mask THEN
        ...
        pf.setvalue(ax, y, pf.byte_order.B, c.b);
      ELSE
        pf.blend_pix_bgr24_pre(ax, y, c.r, c.g, c.b, alpha, covers^[i + cov_offset]);
      END;
...
PROCEDURE (pf: pixel_format_bgr24_pre_ptr) blend_solid_vspan*(x, y, len: bas.int32; c: col.aggclr;
  ...
  IF c.a # 0 THEN
    ...
    REPEAT
      ...
      IF alpha = col.base_mask THEN
        ...
        pf.setvalue(x, ay, pf.byte_order.B, c.b);
      ELSE
        pf.blend_pix_bgr24_pre(x, ay, c.r, c.g, c.b, alpha, covers^[i + cov_offset]);
      END;
...
PROCEDURE (pf: pixel_format_bgr24_pre_ptr) blend_color_hspan*(x, y, len: bas.int32;
...
BEGIN
  IF covers # NIL THEN
    ...
    ax := x;
    REPEAT
      pf.copy_or_blend_pix_bgr_pre(ax, y, colors^[i + col_offset], covers^[i + cov_offset]);
      INC(i);
      INC(ax);
    UNTIL i = alen;
  ELSE
    ...
    IF cover = bas.cover_full THEN
      ...
      ax := x;
      REPEAT
        pf.copy_or_blend_pix_bgr1(ax, y, colors^[i + col_offset]);
        INC(i);
        INC(ax);
      UNTIL i = alen;
    ELSE
      ...
      ax := x;
      REPEAT
        pf.copy_or_blend_pix_bgr_pre(ax, y, colors^[i + col_offset], cover);
        INC(i);
        INC(ax);
      UNTIL i = alen;
    END;
...
PROCEDURE (pf: pixel_format_bgr24_pre_ptr) blend_color_vspan*(x, y, len: bas.int32;
  VAR colors: col.colors_array_ptr; col_offset: bas.int32;
  VAR covers: col.covers_array_ptr; cov_offset: bas.int32;
       cover: bas.int8u);
VAR
...
BEGIN
  IF covers # NIL THEN
    ...
    ay := y;
    REPEAT
      pf.copy_or_blend_pix_bgr_pre(x, ay, colors^[i + col_offset], covers^[i + cov_offset]);
      INC(i);
      INC(ay);
    UNTIL i = alen;
  ELSE
    ...
    IF cover = bas.cover_full THEN
      ...
      ay := y;
      REPEAT
        pf.copy_or_blend_pix_bgr1(x, ay, colors^[i + col_offset]);
        INC(i);
        INC(ay);
      UNTIL i = alen;
    ELSE
      ...
      ay := y;
      REPEAT
        pf.copy_or_blend_pix_bgr_pre(x, ay, colors^[i + col_offset], cover);
        INC(i);
        INC(ay);
      UNTIL i = alen;
    END;
...
PROCEDURE (pf: pixel_format_bgr24_pre_ptr) copy_from*(from: rb.rendering_buffer_ptr; xdst, ydst, xsrc, ysrc, len: bas.int32);
BEGIN
  bas.agg_move(
...
PROCEDURE (pf: pixel_format_bgr24_pre_ptr) blend_from*(from: AggPixfmtBase.pixel_formats_ptr; xdst, ydst, xsrc, ysrc, len: bas.int32; cover: bas.int8u);
VAR
  color: col.aggclr;
  alpha: bas.int8u;
...
BEGIN
  ...
  ELSE
    REPEAT
      ...
      pf.copy_or_blend_pix_bgr_pre(xd, yd, color, cover);
