...
TYPE
  G_SQRT_TABLE* = ARRAY 1024 OF bas.int16u;
  G_ELDER_BIT_TABLE* = ARRAY 256 OF bas.int8;

VAR
  g_sqrt_table*: G_SQRT_TABLE;
  g_elder_bit_table*: G_ELDER_BIT_TABLE;

CONST
  intersection_epsilon* = 1.0E-30;

PROCEDURE calc_point_location*(x1, y1, x2, y2, x, y: bas.double ): bas.double;
BEGIN
   RETURN (x - x2) * (y2 - y1) - (y - y2) * (x2 - x1);
END calc_point_location;

PROCEDURE point_in_triangle*(x1, y1, x2, y2, x3, y3, x, y: bas.double ): BOOLEAN;
VAR
 cp1, cp2, cp3: BOOLEAN;
BEGIN
...
PROCEDURE calc_distance*(x1, y1, x2, y2: bas.double): bas.double;
VAR
  dx, dy: bas.double;
BEGIN
  dx := x2 - x1;
...
PROCEDURE calc_line_point_distance*(x1, y1, x2, y2, x, y: bas.double): bas.double;
VAR
  d, dx, dy: bas.double;
BEGIN
  dx := x2 - x1;
  ...
  END;
END calc_line_point_distance;

PROCEDURE calc_intersection*(ax, ay, bx, by, cx, cy, dx, dy: bas.double; VAR x, y: bas.double) : BOOLEAN;
VAR
  r, num, den: bas.double;
BEGIN
  ...
  ELSE
    ...
    RETURN TRUE;
  END;
END calc_intersection;

PROCEDURE intersection_exists*(x1, y1, x2, y2, x3, y3, x4, y4: bas.double): BOOLEAN;
VAR
 dx1, dy1, dx2, dy2: bas.double;
BEGIN
  dx1 := x2 - x1;
  dy1 := y2 - y1;
  dx2 := x4 - x3;
...
END intersection_exists;

PROCEDURE calc_orthogonal*(thickness, x1, y1, x2, y2: bas.double; VAR x, y: bas.double);
VAR
  d, dx, dy: bas.double;
BEGIN
  dx := x2 - x1;
...
PROCEDURE dilate_triangle*(x1, y1, x2, y2, x3, y3: bas.double; VAR x, y: bas.double_8; d: bas.double);
VAR
  loc, dx1, dy1, dx2, dy2, dx3, dy3: bas.double;
BEGIN
  dx1 := 0.0;
  dy1 := 0.0;
  dx2 := 0.0;
...
PROCEDURE calc_triangle_area*(x1, y1, x2, y2, x3, y3: bas.double): bas.double;
BEGIN
  RETURN (x1 * y2 - x2 * y1 + x2 * y3 - x3 * y2 + x3 * y1 - x1 * y3) * 0.5;
END calc_triangle_area;

PROCEDURE fast_sqrt*(val: bas.int32): bas.int32;
VAR
  bit: bas.int32;
  t, shift: bas.int32;
  s: SET;
BEGIN
  ...
  bit := bas.shr_int32(t, 24);

  IF bit # 0 THEN
    bit := g_elder_bit_table[bit] + 24
  ELSE

    bit := bas.shr_int32(t, 16);
    s := sys.VAL(SET, bit);
    s := s * {0..7};
    bit := sys.VAL(bas.int32, s);

    IF bit # 0 THEN
      bit := g_elder_bit_table[bit] + 16
    ELSE
      bit := bas.shr_int32(t, 8);
      s := sys.VAL(SET, bit);
      s := s * {0..7};
      bit := sys.VAL(bas.int32, s);
...
(* This is calculation sqrt itself.*)
  ...
  IF bit > 0 THEN
    ...
    shift := 11 - bit;
    val   := bas.shr_int32(val, (ASH(bit, 1)));
  ELSE
    shift := 11;
  END;

  RETURN bas.shr_int32(g_sqrt_table[val], shift);
...
------------------------------------------------------------------------
*)
PROCEDURE besj*(x: bas.double; n: bas.int32): bas.double;
VAR
  i, m1, m2, m8, imax: bas.int32;
  b, b1, c2, c3, c4, c6: bas.double;
...
BEGIN
  ...
  IF ABS(x) > 5 THEN
    m1 := ENTIER(ABS(1.4 * x + 60 / x));
  END;

  m2 := ENTIER(n + 2 + ABS(x) / 4);
